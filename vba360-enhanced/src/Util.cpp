...
#ifndef NO_PNG
// Custom PNG write function for HANDLE instead of FILE*
static void png_write_handle(png_structp png_ptr, png_bytep data, png_size_t length)
{
  HANDLE hFile = (HANDLE)png_get_io_ptr(png_ptr);
  DWORD bytesWritten;
  WriteFile(hFile, data, (DWORD)length, &bytesWritten, NULL);
}

// Custom PNG flush function for HANDLE
static void png_flush_handle(png_structp png_ptr)
{
  // Flush is not needed for HANDLE, but function is required
  (void)png_ptr;
}
#endif
...
bool utilWritePNGFile(const char *fileName, int w, int h, u8 *pix)
{
  ...
  png_set_write_fn(png_ptr, (void*)fp, png_write_handle, png_flush_handle);
